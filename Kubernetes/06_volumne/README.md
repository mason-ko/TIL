...
> PVC에서 존재하지 않은 스토리지클래스를 참조하면 PV 프로비저닝 실패 ( k describe 명령으로보면 ProvisioningFailed 이벤트 표시 )
...
관리자: 성능 특성이 다른, 여러 스토리지 클래스 생성  
관리자: 생성할 각 클레임에 가장 적합한 스토리지 클래스 결정  

스토리지클래스의 장점은 클레임 이름으로 참조한다는 것.  
### 스토리지 클래스를 지정하지 않은 동적 프로비저닝 
gke 에서 사용 가능한 스토리지 클래스 조회
```
$ kubectl get sc
```
### 기본 스토리지 클래스 확인하기 
...
storageClassName 속성을 지정하지 않고 PVC를 생성하면 GKE에서는 pd-standard 유형의 GCE 퍼시스턴트 디스크가 프로비저닝 된다.

### PVC를 미리 프로비저닝된 PV로 바인딩 강제하기 
storageClassName 속성을 빈 문자열로 지정 시 동적 볼륨 프로비저너는 새로운 PV를 프로비저닝 한다.

### PV 동적 프로비저닝의 전체그림 이해하기 
요약하면 파드에 퍼시스턴트 스토리지를 연결하는 최적의 방법은 PVC 와 파드만 생성하는 것이다.  
왜?

퍼시스턴트 볼륨 클레임(Persistent Volume Claim, PVC)을 사용하고 파드를 생성하는 것이 파드에 퍼시스턴트 스토리지를 연결하는 최적의 방법인 이유는 다음과 같습니다
- 자원 추상화: PVC를 사용하면, 사용자는 복잡한 스토리지 인프라에 대해 신경 쓸 필요 없이 필요한 스토리지 용량과 액세스 모드를 정의하면 됩니다. 이렇게 하면 Kubernetes 시스템이 적절한 퍼시스턴트 볼륨(PV)을 찾아 바인딩해줍니다.
- 스토리지 자동 프로비저닝: PVC를 정의하면, 동적 프로비저닝이 설정된 경우 Kubernetes는 자동으로 필요한 크기와 속성을 가진 PV를 생성하고 바인딩합니다. 이는 수동으로 PV를 생성하고 관리하는 수고를 덜어줍니다.
- 포터블: PVC를 사용하면, 파드를 다른 노드로 이동하거나 다른 클러스터로 마이그레이션하는 등의 상황에서도 파드와 스토리지 간의 연결이 유지됩니다. 이는 애플리케이션의 이식성을 향상시킵니다.
- 보안과 격리: PVC를 사용하면, 각 파드 또는 애플리케이션에 필요한 스토리지를 따로 할당하고 관리할 수 있습니다. 이는 각 파드 간에 데이터를 격리시키므로 보안을 향상시킵니다.
따라서, PVC를 사용하고 파드를 생성하는 것은 퍼시스턴트 스토리지를 자동으로, 효율적으로, 안전하게 관리하고 파드에 연결하는 간편하고 유연한 방법을 제공합니다. 이 방법은 Kubernetes의 기본적인 스토리지 관리 패러다임을 따르므로, 클러스터에서 스토리지를 사용하는 가장 일반적이고 권장하는 방법입니다.  

![image](https://github.com/mason-ko/TIL/assets/30224146/768542f9-8411-4ad9-ba13-3cfc551eb49a)


## [6.7 요약](#index)
- 다중 컨테이너 파드 생성과 파드의 컨테이너들이 볼륨을 파드에 추가하고 각 컨테이너에 마운트해 동일한 파일로 동작하게 한다.
- emptyDir 볼륨을 사용해 임시, 비영구 데이터를 저장한다.
- gitRepo 볼륨을 사용해 파드의 시작 시점에 깃 레포의 콘텐츠로 디렉터리를 쉽게 채운다.
- hostPath 볼륨을 사용해 호스트 노드의 파일에 접근한다.
- 외부 스토리지를 볼륨에 마운트해 파드가 재시작돼도 파드의 데이터를 유지한다.
- PV와 PVC를 사용해 파드와 스토리지 인프라스트럭쳐를 분리한다.
- 각 PVC를 위해 PV를 원하는 스토리지 클래스로 동적 프로비저닝한다.
- PVC를 미리 프로비저닝된 PV와 바인딩하고자 할때 동적 프로비저너가 간섭하는 것을 막는다.
